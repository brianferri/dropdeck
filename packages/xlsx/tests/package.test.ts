...
await test("package: an external relationship carries TargetMode", () => {
    const part = relationship("rId9", RelationshipType.Worksheet, "https://example.com", TargetMode.External);
    assert.equal(
        xml(part),
        `<Relationship Id="rId9" Type="${RelationshipType.Worksheet}" Target="https://example.com" TargetMode="External"/>`
    );
});
...
await test("package: content types list defaults and overrides", () => {
    const part = contentTypes([
        ...
        override("/xl/workbook.xml", ContentType.Workbook)
    ]);
    assert.equal(
        xml(part),
        [
...
await test("package: pack writes [Content_Types].xml first and every part path", async () => {
    ...
    assert.ok(whole.includes("xl/workbook.xml"));
});
