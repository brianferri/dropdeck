...
import {
    CALLEE_BY_GLYPH, CONSTANT_BY_GLYPH, MATH_TOKEN_BY_GLYPH, isCalleeGlyph, isConstantGlyph, isTokenGlyph
...
function toMathNary(callee: string, children: ReadonlyArray<Notation>): string {
    const [lower, upper, body] = children;
    if (lower.kind !== NotationKind.Row || lower.children.length !== 3) throw new Error(`a math ${callee} needs an 'index = start' lower limit`);
...
function toMathNode(node: Notation): string {
    switch (node.kind) {
        ...
        case NotationKind.Nary: {
            ...
            return toMathNary(CALLEE_BY_GLYPH[node.symbol], node.children);
        }
